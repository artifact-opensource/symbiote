# Symbiote Releases

## Latest: v1.6.0 (March 7, 2026)
Native Gemini provider, 8 LLM providers, multi-user deployment. See [RELEASE_NOTES.md](RELEASE_NOTES.md) for details.

## All Releases

| Version | Date | Highlights |
|---------|------|-----------|
| **v1.6.0** | Mar 7, 2026 | Native Gemini, 8 providers, multi-user, de-branded UI |
| **v1.5.0** | Mar 6, 2026 | Blink, Pulse, COMB, 7 providers, agent wizard |
| **v1.4.0** | Mar 5, 2026 | MCP server, anti-loop, degradation protection |
| **v1.3.0** | Mar 3, 2026 | Multi-bot coordination, ATM, sibling yield |
| **v1.2.0** | Feb 28, 2026 | Multi-bot coordination, echo loop prevention |
| **v1.1.0** | Feb 28, 2026 | Brand kit, CLI wizard, first npm publish |
| **v1.0.0** | Feb 28, 2026 | First stable: Discord, WhatsApp, HTTP API, 18 tools, 4 providers |

## Changelog

Full changelog with detailed breaking changes, fixes, and migration notes: [docs/reference/changelog.md](docs/reference/changelog.md)
